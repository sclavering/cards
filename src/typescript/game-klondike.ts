class _Klondike extends Game {
  ...
  _get_score(act: Move): number {
    const s = act.cseq.source, d = act.destination;
    ...
    if(s instanceof Waste && d instanceof Foundation) {
      for(let p of this.piles)
        if(p.may_add(act.cseq))
          return 15;
      return 10;
    }
    if(d instanceof Foundation) return s instanceof Foundation ? 0 : 10;
    if(s instanceof Foundation) return -15;
...
class KlondikeDrawThreeGame extends _Klondike {
  static create_layout() {
    return new Layout("#<    s w  f f f f    >.#<   p p p p p p p   >.", { w: DealThreeWasteHorizontalView });
  }
  constructor() {
    super();
    this.pile_details = {
      stocks: [1, StockDealThreeOrRefill, 0, 0],
      wastes: [1, DealThreeWaste, 0, 0],
      piles: [7, KlondikePile, [0,1,2,3,4,5,6], 1],
      foundations: [4, KlondikeFoundation, 0, 0],
    };
  }
