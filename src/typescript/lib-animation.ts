...
class g_animations {
  ...
  private static _args: AnimationDetails;
  private static _onsuccess: () => void;
  private static _timeouts: number[];
...
class g_floating_pile {
  ...
  private static _prev_card: Card;
  ...
  static bounding_rect(): ClientRect {
  ...
  static start_freecell_animation(src: AnyPile, cards_remaining: Card[], moving_card: Card, x: number, y: number): void {
    const view = src.view;
    view.draw_into(this._context, [moving_card], false);
    this.set_position(x, y);
    view.update_with(cards_remaining);
  }

  static set_position(x: number, y: number): void {
    this._canvas.style.left = x + "px";
    this._canvas.style.top = y + "px";
  }

  static transition_from_to(x0: number, y0: number, x1: number, y1: number, duration_ms: number): void {
    this.set_position(x0, y0);
    this._canvas.style.transition = "left " + duration_ms + "ms, top " + duration_ms + "ms";
    this.set_position(x1, y1);
  }

  static get_transition_duration_ms(x0: number, y0: number, x1: number, y1: number): number {
